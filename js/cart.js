...
const cart = (function() {
  function createItemToCart(productId, quantity, thumbImageURL, productName, discountPrice, totalPrice) {
    return {
      item_id: productId,
      quantity,
      thumb_URL: thumbImageURL, // thumbImageURL ni thumb_URL ga o'zgartirdik
      name: productName,
      discount_price: discountPrice,
      total_price: totalPrice
    };
  ...
  function loadAllItems() {
    const items = [];
    let sessionStorageLength = sessionStorage.length;
    for (let i = 0; i < sessionStorageLength; i++) { // <= dan < gacha o'zgaruvchini o'zgartirdik
      let item = sessionStorage.getItem(sessionStorage.key(i)); // item = sessionStorage.getItem(`item-cart-${i}`) ni o'zgartirdik
      if (item !== null) {
        item = JSON.parse(item);
        items.push(item);
      }
